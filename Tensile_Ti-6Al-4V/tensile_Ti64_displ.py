...
bc = [DirichletBC(V.sub(0), 0, boundary_L), DirichletBC(V.sub(1), 0, boundary_L),  DirichletBC(V.sub(2), 0, boundary_L), DirichletBC(V.sub(1),loading,boundary_R)]
...
def eps_MFront(v):
    e = sym(grad(v))
    return as_vector([e[0, 0], e[1, 1], 0, sqrt(2)*e[0, 1], sqrt(2)*e[0,2], sqrt(2)*e[1,2]])
...
Nitermax, tol = 20, 1e-5  # parameters of the Newton-Raphson procedure
Nincr = 10
...
for (i, t) in enumerate(load_steps):
    ...
    nRes = nRes0
    u1.assign(u)
    print("Increment:", str(i+1), " , t:", t*800)
    niter = 0
    while nRes/nRes0 > tol and niter < Nitermax:
        print("    t = ", t, ", loading = ", loading)
